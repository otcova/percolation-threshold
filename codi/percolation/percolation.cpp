...
Graph node_percolation(Graph &graph, float q) {
  ...
  for (auto it_u = diff_accum.begin(); it_u != diff_accum.end(); ++it_u) {
    ...
    vector<int> adjacent = graph.adjacent_nodes(
        it_u->first); // adjacent nodes to surviving node in old graph
    for (int v : adjacent) {
      auto it_v =
      ...
      if (it_v != diff_accum.end()) // v is a surviving node
      {
        ...
        if (new_v > new_u)
          new_graph.add_edge(new_u, new_v); // add edge with updated node "id"
      }
    }
  }
  /*
  cout << "surviving nodes: " << endl;
  for (auto it = diff_accum.begin(); it != diff_accum.end(); ++it) cout <<
  it->first << " "; cout << endl; cout << "old graph" << endl;
  // graph.print_graph();
  cout << "new graph" << endl;
