...
Graph node_percolation(Graph &graph, float q) {
  ...
  for (int u = 0; u < old_n_nodes; ++u) 
    if (dist100(rng)/float(100) <= q) surviving_nodes.push_back(u);
  ...
  {
    int diff_aux = surviving_nodes[u] - surviving_nodes[u-1] - 1 + last_diff_accum;
  ...
  {
    vector<int> adjacent = graph.adjacent_nodes(surviving_nodes[u]); //adjacent nodes to surviving node in old graph
    for (int v : adjacent)
    {
      auto it = diff_accum.find(v); //iterator points to diff_accum of v, or diff_accum.end() if it's not a surviving node
      if (it != diff_accum.end()) // v is a surviving node
      {
        int new_v = v - diff_accum[v];
        if (new_v > u)  new_graph.add_edge(u, new_v); //add edge with updated node "id"
      }
    }  
  }
  cout << "surviving nodes: " << endl;
  for (int u : surviving_nodes) cout << u << " ";
  cout << endl;
  cout << "old graph" << endl;
  graph.print_graph();
  cout << "new graph" << endl;
